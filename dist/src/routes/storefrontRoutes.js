...
import { createOrder, getOrder, getOrderAnywhere, orderItems, } from '../services/orderService.js';
import { upsertTelegramUser, createUser } from '../services/userService.js';
import { createPaymentForOrder, getPaymentByOrder, submitManualReceipt, attemptTonVerification, } from '../services/paymentService.js';
...
storefrontRoutes.get('/', async (c) => {
    ...
    if (stores.length === 1)
        return c.redirect(`/store/${stores[0].slug}`);
...
storefrontRoutes.post('/api/order/:orderId/receipt', async (c) => {
    ...
    if (!payment || payment.method !== 'MANUAL')
