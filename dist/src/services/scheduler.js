...
async function pollTonPayments() {
...
function register(jobKey, schedule, handler) {
...
export function startScheduler() {
    ...
    logger.info({ enableInlineWorker: inlineOnly, enableBackups: env.worker.enableBackups }, 'scheduler started');
    // Backup job.
    if (env.worker.enableBackups) {
...
export async function dbHeartbeat() {
    try {
        const row = await queryOne('SELECT 1 AS n');
        return row?.n === 1;
    }
    catch {
        return false;
