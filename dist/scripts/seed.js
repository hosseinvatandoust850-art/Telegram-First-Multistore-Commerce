...
async function main() {
    if (isProd && !force) {
        console.error('[seed] Refusing to run in production. Production data is created via ' +
    ...
    try {
        ...
        console.error('[seed] migrations failed:', err.message);
    }
    ...
    if (!storeRow) {
        ...
        const res = await pool.query(`INSERT INTO "Store" (slug, name, description, currency, "currencySymbol")
       VALUES ($1,$2,$3,'USDT','₮') RETURNING id`, [slug, 'Demo Store', 'A demo store for development.', 'https://demo.local']);
        const storeId = res.rows[0].id;
        await pool.query(`INSERT INTO "Product" ("storeId", name, slug, type, price, currency, description, active)
       VALUES ($1,'Demo Digital Product','demo-digital','DIGITAL',10,'USDT','Instant digital delivery.',true)
        ,($1,'Demo Physical Item','demo-physical','PHYSICAL',25,'USDT','A physical demo item.',true)`, [storeId]);
