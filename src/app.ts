...
export function buildApp(): Hono {
  ...
  app.get('/storage/*', async (c) => {
    if (env.storage.type === 's3') return c.notFound();
    ...
    const ct =
      { '.png': 'image/png', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.pdf': 'application/pdf', '.gif': 'image/gif', '.webp': 'image/webp' }[ext] ||
  ...
  app.onError((err, c) => {
    ...
    return c.json(
      { ok: false, error: 'Internal server error', code: 'INTERNAL' },
      500,
    );
  });
