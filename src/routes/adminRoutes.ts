...
function storeIdOf(c: Parameters<typeof getUser>[0]): string {
  const user = getUser(c)!;
  const headerStore = c.req.header('x-store-id');
  if (user.role === 'SUPER_ADMIN' && headerStore) return headerStore;
  if (!user.storeId) throw new ForbiddenError('No store associated with this account');
  return user.storeId;
}
...
adminRoutes.post('/admin/payments/:id/review', async (c) => {
  ...
  if (!payment || payment.storeId !== storeId) throw new NotFoundError('Payment not found');
...
adminRoutes.patch('/admin/store', async (c) => {
  ...
  const store = await updateStore(storeId, patch as any);
