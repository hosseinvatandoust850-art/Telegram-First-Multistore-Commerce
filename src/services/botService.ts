...
import { TelegramBot, type TgMessage, type TgMessageContent } from './telegram.js';
...
export function webhookUrlForStore(store: { botWebhookSecret?: string | null }): string {
...
export async function registerStoreWebhook(
  store: { id: string; telegramBotToken?: string | null; botWebhookSecret?: string | null },
): Promise<{ ok: boolean; url: string; info?: unknown }> {
...
export async function registerMasterWebhook(): Promise<{ url: string; ok: boolean }> {
  const bot = masterBot();
...
export function parseUpdate(body: string): TgMessage {
  try {
    return JSON.parse(body) as TgMessage;
  } catch {
    return {};
  }
}

function textOf(msg: TgMessageContent | undefined): string | undefined {
  return msg?.text;
}

/** Handle an incoming update from the store webhook route. */
export async function handleStoreUpdate(
  store: { id: string; name: string; slug: string; telegramBotToken?: string | null },
  update: TgMessage,
  ...
  const msg = update.message || (update.callback_query?.message as TgMessageContent | undefined);
  if (!msg) return { replied: false };

  const sender = msg.from;
  if (!sender || sender.is_bot) return { replied: false };
  ...
  if (text && text.length > 1) {
    ...
    if (products.length > 0) {
      ...
      await bot.sendMessage(
        chatId,
        `🔎 Found: <b>${escapeHtml(p.name)}</b>\n` +
          `${p.price} ${p.currency}\n${p.description ? escapeHtml(p.description) : ''}\n` +
          'Buy here: ' +
          `${env.app.publicUrl}/store/${store.slug}/product/${p.slug}`,
        { parse_mode: 'HTML' },
      );
      return { replied: true };
    }
...
export async function notifyStoreOwner(
  store: { telegramUsername?: string | null; telegramBotToken?: string | null },
  text: string,
): Promise<void> {
  const bot = botForStore(store);
  if (!bot) return;
...
export async function handleMasterUpdate(update: TgMessage): Promise<{ replied: boolean }> {
  const bot = masterBot();
  if (!bot) return { replied: false };
  ...
  if (text.startsWith('/start')) {
    await bot.sendMessage(
      chatId,
      '👋 Welcome! This is the Multistore Commerce platform bot.\n\n' +
        'Use the buttons or your store to start shopping.',
      { parse_mode: 'HTML' },
    );
...
export async function resolveStoreFromSecret(secret: string) {
