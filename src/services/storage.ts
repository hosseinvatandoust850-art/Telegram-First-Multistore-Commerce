...
function localSafePath(prefix: string, name: string): string {
  const base = dataRoot();
...
export async function readLocalFile(key: string): Promise<Buffer | undefined> {
  if (env.storage.type === 's3') return undefined; // handled by presigned URL
  try {
...
export async function localFileSize(prefix?: string): Promise<number> {
  const base = dataRoot();
  try {
