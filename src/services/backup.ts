 ...
 */
export async function createBackup(): Promise<BackupJobRow> {
  const url = env.db.directUrl || env.db.url;
  if (!url) throw new Error('DATABASE_URL is required for backups');

  const started = await queryOne<BackupJobRow>(
    `INSERT INTO "BackupJob" (status, type) VALUES ('RUNNING', 'full') RETURNING *`,
  );
  ...
  try {
    ...
    if (env.worker.backupS3) {
      ...
      await queryOne<BackupJobRow>(
        `UPDATE "BackupJob" SET status = 'COMPLETED', "storageKey" = $2,
           "sizeBytes" = $3, "completedAt" = now() WHERE id = $1 RETURNING *`,
        [jobId, key, BigInt(buf.length).toString()],
      );
      await pruneBackups();
    ...
    await query(
      `UPDATE "BackupJob" SET status = 'COMPLETED', "storageKey" = $2,
         "sizeBytes" = $3, "completedAt" = now() WHERE id = $1`,
      [jobId, `backups/${filename}`, BigInt(size).toString()],
    );
    await pruneBackups();
    return (await getBackupJob(jobId))!;
  } catch (err) {
    ...
    await query(
      `UPDATE "BackupJob" SET status = 'FAILED', error = $2, "completedAt" = now() WHERE id = $1`,
...
export async function getBackupJob(id: string): Promise<BackupJobRow | undefined> {
  return queryOne<BackupJobRow>('SELECT * FROM "BackupJob" WHERE id = $1', [id]);
}

export async function listBackups(limit = 20): Promise<BackupJobRow[]> {
  return query<BackupJobRow>(
...
async function pruneBackups(): Promise<void> {
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (s && item.isFile()) files.push({ name: item.name, mtime: s.mtimeMs });
    }
